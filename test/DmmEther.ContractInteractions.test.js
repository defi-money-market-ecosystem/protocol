...
describe('DmmEther.ContractInteractions', async () => {
  ...
  /********************************
   * Redeem from to WETH Functions
   */

  it('should redeemFromToWETH to sender via admin if wallet is set up', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(admin, constants.MAX_UINT256, {from: user});
    ...
    const gasPrice = new BN(1e9);
    const receipt = await this.contract.redeemFromToWETH(user, admin, _25(), {from: admin, gasPrice});
    expectEvent(
      receipt,
      'Redeem',
      {redeemer: user, recipient: admin, amount: _25()}
    ...
    (await balance.current(admin)).should.be.bignumber.equal(adminBalance.sub(totalGasUsed));
  });

  it('should redeemFromToWETH if market is disabled', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await disableMarkets(this.controller, admin);
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
  ...
  it('should not redeemFromToWETH if sender is not approved', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(admin, constants.MAX_UINT256, {from: user});
    ...
    await expectRevert(
      this.contract.redeemFromToWETH(user, other, _25(), {from: other}),
      'INSUFFICIENT_ALLOWANCE'
    )
  });

  it('should not redeemFromToWETH if ecosystem is paused', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await pauseEcosystem(this.controller, admin);
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    ...
    await expectRevert(
      this.contract.redeemFromToWETH(user, admin, _25(), {from: admin}),
      'ECOSYSTEM_PAUSED'
    )
  });
  ...
  it('should not redeemFromToWETH if user is blacklisted', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(admin, constants.MAX_UINT256, {from: user});
  ...
  it('should not redeemFromToWETH if msg.sender is blacklisted', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(admin, constants.MAX_UINT256, {from: user});
  ...
  it('should not redeemFromToWETH if receiver is blacklisted', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(admin, constants.MAX_UINT256, {from: user});
    ...
    await expectRevert(
      this.contract.redeemFromToWETH(user, other, _25(), {from: admin}),
      'BLACKLISTED'
    );
  });

  it('should not redeemFromToWETH if amount is too small', async () => {
    await mint(this.underlyingToken, this.contract, user, _25());
    await this.underlyingToken.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(this.contract.address, constants.MAX_UINT256, {from: user});
    await this.contract.approve(admin, constants.MAX_UINT256, {from: user});
    ...
    await expectRevert(
      this.contract.redeemFromToWETH(user, admin, new BN('1'), {from: admin}),
      'INSUFFICIENT_REDEEM_AMOUNT'
    );
  });
