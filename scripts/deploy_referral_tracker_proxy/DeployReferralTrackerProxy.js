...
const main = async () => {
  ...
  const ReferralTrackerProxyFactory = loader.truffle.fromArtifact('ReferralTrackerProxyFactory');
  const dmgToken = await deployContract(
    ReferralTrackerProxyFactory,
    [weth],
    deployer,
    2.5e6,
    web3,
  ...
  console.log(`Deployed referral tracker proxy factory with ${gasUsed} gas`)
};
